...
3. 커밋되지 않은 변경이 있으면 `git add -A` 후 커밋한다. 메시지: 첫 줄 `type: 무엇을 (GFM-n)`(한국어, 50자 내), 본문에 왜·확인 방법, 마지막에 트레일러
   `Co-Authored-By: Claude <이 세션의 실제 모델명> <noreply@anthropic.com>`.
4. `git push -u origin <브랜치>`.
5. 이미 열린 PR이 있으면(`gh pr view --json url`) 새로 만들지 않고 URL만 보여준다. 없으면 `gh pr create --base develop` — **제목 = squash 커밋 제목**(`type: 무엇을 (GFM-n)`), 본문은 `## 무엇을 / 왜`, `## 확인`(로컬에서 본 것), `## 후속`, `Jira: GFM-n`, 그리고 `🤖 Generated with [Claude Code](https://claude.com/claude-code)`.
6. Jira 이슈에 인수인계 코멘트를 남긴다(claude-code.md §5 템플릿: 브랜치/PR, 한 것, 남은 것, 막힌 것). 상태를 `검토 중`(transition 31)으로 바꾼다.
7. PR URL과 "병합은 core·Owner 가 `/merge`(또는 웹 'Bypass rules and merge')로 squash, contributors 는 core 에게 요청, 병합 후 dev `/version`에서 확인"을 한 줄로 안내하고 멈춘다. **이 명령은 병합하지 않는다.**
