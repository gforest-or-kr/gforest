...
### 5.6 보안 모델 — 변경점과 "S3 통상 동작"이라는 점

> **요약**: 객체 키(주소)만 알아도 접근 불가하고 회원 권한은 그대로 강제된다. 단 Supabase Storage가 주던 *저장소 레이어 RLS 백스톱*은 사라진다. **이 변경은 R2만의 단점이 아니라 모든 S3 호환 객체 스토리지(AWS S3·R2·B2·Wasabi·MinIO 등)의 통상 동작이다** — 백스톱은 표준 S3엔 원래 없고 Supabase가 덧붙인 고유 기능이었다.

**모든 S3 호환 스토리지에 공통인 접근 모델:**
- 버킷 기본 **비공개** + 자격증명 또는 **presigned URL**(서명+만료)로만 접근. **객체 키만으로는 접근 불가** — 비공개 버킷은 서명 없는 요청을 거부한다.
- 스토리지는 우리 앱의 회원/역할/RLS를 **모른다** → 권한은 **앱 레이어에서 강제**해야 한다.
- 공개 접근은 명시적 opt-in(공개 버킷/도메인). presigned URL을 만료 안에 복사·공유하면 그 창 동안 접근 가능 — 이 한계도 공통(**Supabase 서명 URL도 동일**).

**우리 설계에서의 강제 경로(`app/dl/[id]/route.ts`):** ① 첨부 행을 Supabase RLS로 조회(`can_read_board`) → 권한 없으면 404 → ② 통과 시에만 presigned 발급 → 302. **①은 R2로 바뀌어도 그대로**(행은 Postgres에 있음). 본문 HTML엔 `/dl/{id}` 프록시 링크만 노출되고 R2 직접 주소는 절대 노출되지 않는다 → **비회원이 글/링크 주소를 알아도 프록시가 RLS로 막는다.**

**Supabase Storage 대비 차이 (방어막 한 겹 감소):**

| | 권한 판정 위치 | 저장소 레이어 RLS 백스톱 |
|---|---|---|
| Supabase Storage | 앱 `/dl` 프록시 + `storage.objects` RLS | **있음 (Supabase 고유 기능)** |
| R2 / AWS S3 / B2 / 일반 S3 | **앱 `/dl` 프록시 + `attachments` 테이블 RLS만** | **없음 (표준 S3엔 원래 부재)** |

- Supabase는 권한이 2중(프록시 + 저장소 RLS)이라 프록시에 버그가 나도 storage RLS가 최후 차단. **R2는 그 백스톱이 없어** 판정이 앱 한 곳에 의존.
- AWS S3는 통제 수단(bucket policy·IAM·ACL·STS·Cognito)이 더 많지만 전부 **AWS IAM 신원에 묶인다.** 우리는 Supabase Auth를 쓰므로 결국 프록시에서 다시 판정 → **우리 아키텍처에선 R2와 실질 동일.** 차이는 보안이 아니라 egress 비용뿐.

**완화책(R2/일반 S3 채택 시 공통 적용):**
- 권한 체크를 `/dl` 프록시 한 곳에 집중 — 직접 버킷 URL 우회 경로를 본문·API 어디에도 만들지 않음
- **private/public 버킷 물리 분리** — 회원 콘텐츠는 반드시 private 버킷. (회원 첨부를 공개 버킷에 두면 키만으로 노출 = 치명)
- 민감 첨부는 presigned 만료를 짧게(60~300초)
- 저장소 백스톱이 사라진 만큼 **유일한 방어선인 `attachments_select` RLS 정합성을 CI 회귀 테스트로 보강**
