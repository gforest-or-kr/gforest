...
- `CLAUDE.md`(원칙)와 `.claude/settings.json`(허용 명령·자동 메모리 끔)은 저장소에 있어 자동 적용된다. 개인 설정은 `.claude/settings.local.json`, **개인 취향(호칭·말투·출력 형식·내 머신 경로)은 `CLAUDE.local.md`** — `cp CLAUDE.local.md.example CLAUDE.local.md` 로 만들어 채운다(둘 다 gitignore). 팀이 알아야 할 사실은 여기가 아니라 repo 문서에 PR 로.
...
- [ ] (Claude) `CLAUDE.local.md` 를 만들었고, 이전에 이 저장소로 쌓인 자동 메모리가 있으면 팀 사실은 PR 로 옮기고 나머지는 지움
