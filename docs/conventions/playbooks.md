...
4. 화면 버그, 특히 레거시 본문·모바일 레이아웃은 **WebKit(Playwright webkit, 390px)** 으로도 재현·확인한다. Chromium 에서 안 보이는 버그가 iPhone 에서 보인다([code-patterns.md](./code-patterns.md) §7).
5. 나머지는 §3 과 같다. prod 가 지금 아프면 §5.
