...
- 버전은 **날짜(CalVer)** 다. API 소비자가 없는 홈페이지라 `1.2.3` 식 의미 버전은 결정만 늘린다. dev 의 `/version` 은 `v2026.09.05-3-gabc1234`(마지막 릴리스 이후 3 커밋) 형태 — 가운데 숫자는 git 이 세므로 개발자가 손댈 것이 없다. 첫 릴리스 전(태그 없음)에는 `dev-133-gabc1234`(develop 누적 커밋 수). `/version` 의 "리비전" 은 버전이 아니라 ECS 태스크 정의 번호(배포 횟수)다.
