...
- **운영 DB 조회는 `db/tools/dbshell.sh <env> --readonly`** — VPC 안 일회성 postgres 컨테이너 + ECS Exec(playbooks §13). 세션 로그 CloudWatch `/ecs/gforest-exec`.
- **RDS는 비공개**(인터넷 경로 없음). 평소 DB에 닿는 것은 ECS 태스크뿐이다. dbshell 로 안 되는 비상 시(대량 이관·복구)에만
