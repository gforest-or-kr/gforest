...
- 프로토타입(Supabase·Vercel, 2026-06~08)은 **미채택** — 그때 만들어진 데이터는 이관 대상이 아니다(dev 에 남아 있는 것은 개발용 샘플). 이관 원본은 XE 하나뿐이다.
...
2. **최종 이관** — XE 를 읽기 전용으로 전환 → `db/tools/xe` ETL(실데이터 모드) + 첨부 전량 복사 → prod 데이터 검증(건수·샘플). ETL 은 2026-09-05 작성·로컬 검증 완료(원본 실사 결과는 Jira GFM-82).
...
5. **정리** — XE 사이트 정지(최종 덤프·첨부 백업 보관), cafe24 계약 정리, 회원 비밀번호 재설정 안내 발송(SES 정책 확정 후).
