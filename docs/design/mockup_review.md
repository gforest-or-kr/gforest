...
| **렌더링(서버/캐시)** | 🟢 높음 | 시안 A 섹션 대부분 공개 콘텐츠 → 서버 렌더 + 태그 캐시(`unstable_cache`) 적합. 회원 위젯은 세션 기반 서버 렌더(rendering.md §12, 2026-09 AWS 이후) |
...
3. **렌더링 경계**: 공개 콘텐츠는 서버 렌더 + 태그 캐시, 회원 개인화는 세션 기반 서버 렌더로 분리 (rendering.md §12 — 캐시 콜백 안에서 세션을 읽지 말 것).
