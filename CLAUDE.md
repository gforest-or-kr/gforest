...
- **팀 지식은 repo 에, 개인 취향은 로컬에**: 팀이 알아야 할 사실(결정·함정·절차)은 `docs/conventions`·`.claude/`(rules·skills)·`db/README.md` 또는 Confluence 에만 쓴다(PR 로). **이 프로젝트는 Claude 자동 메모리를 꺼 두었다**(`.claude/settings.json`) — 세션 메모리는 머신을 넘지 않아 팀원 간 격차를 만든다. 호칭·말투·출력 형식·내 머신 경로 같은 개인 취향은 `CLAUDE.local.md`(gitignore, 템플릿 `CLAUDE.local.md.example`)에
